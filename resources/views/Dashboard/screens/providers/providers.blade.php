        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex flex-row justify-content-between">
                    ...
                    <a href="{{ route('providers.create') }}" class="btn btn-primary">اضافة مقدم خدمة</a>
                </div>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                    ...
                    <tbody>
                        ...
                        @foreach ($providers as $provider)
                            <tr  data-id="{{$provider->id}}">
                                <td>{{ $provider->id }}</td>
                                <td><img src="{{ asset('storage/'.$provider->image) }}" alt="user image" width="100" height="100"> </td>
                                <td>{{ $provider->name }}</td>
                                ...
                                <td><img src="{{ asset('storage/'.$provider->idCard_photo) }}" alt="user image" width="100" height="100"> </td>

                                <td>{{ $provider->specialization->category->name_ar  . ' / ' . $provider->specialization->name_ar  }} </td>
                                ...
                                <td class="d-flex flex-row justify-content-between g-5"  style="gap: 10px">
                                      ...
                                      <div class="form-group">
                                        <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                                          <input type="checkbox" class="custom-control-input myActiveSwitch" data-id="{{$provider->id}}" data-resource="providers/{{$provider->id}}" id="customSwitch{{$provider->id}}" {{ $provider->isActive == 1 ? 'checked' : '' }}>
                                          <label class="custom-control-label " for="customSwitch{{$provider->id}}"></label>
                                        </div>
                                      </div>

                                  <a href="{{ route('providers.edit',['provider' => $provider->id]) }}" style="color: green;"><i class="fas fa-user-edit"></i> </a>
                                  <a href="#" id="delete" class="delete" style="color: red;" data-id="{{$provider->id}}" data-resource="providers/{{$provider->id}}"><i class="fas fa-trash"></i> </a>
                                 
                                  <a href="{{route('providerInfo', ['provider_id' => $provider->id])}}"><i class="fas fa-download"></i></a>
                                </td>
